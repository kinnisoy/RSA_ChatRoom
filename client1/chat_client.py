...
HOST = 'localhost'
#input('Please input the ip of the server:')
#用本地IP作为本服务器的IP
PORT = 12345
BUFSIZ = 1024
ADDR = (HOST, PORT)
...
class ChatRoomGUI(ChatRoom):
    ...
    def __init__(self,Client, ADDR, name):
        ...
        self.ads = {}
        self.received = b""
        self.send_init_message(pub)#初始化然后发送自己的公钥给服务器
    ...
    def show_self_message(self,msg):#把自己发送的消息打印到自己屏幕
    ...
    def send_message(self):
        msg = self.message_ipt_text.get('0.0', END)#从消息输入框获取消息
...
login_window = LoginWindow(udpCliSock,ADDR)#把socket绑定到登录注册的窗口
...
if state:
    ...
    mthread.start()#开始聊天线程

    mainloop()#关掉聊天窗口，循环结束

    msg = 'disconnect'#告诉服务器我就下线了
